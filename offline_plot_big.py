...
CMD = r'"pscp pi@fhlrasptemp.desy.de:/home/pi/slowpi/data* C:\Users\cordinge\Desktop\RaspberryPi_Sensor-master\3sensors\"' #Putty Command to copy the data from Pi to Windows 
...
fig.canvas.set_window_title('IR Chamber - Humidity and Temperature') #Window Name
...
patch1 = mlines.Line2D([], [], color='red', marker = 's', label='Sensor 1 (Right): Temp') #Legend patches
patch2 = mlines.Line2D([], [], color='red', marker = 'o', label='Sensor 2 (Left): Temp')
patch3 = mlines.Line2D([], [], color='green', marker = 's', label='Sensor 1 (Right): Humidity')
patch4 = mlines.Line2D([], [], color='green', marker = 'o', label='Sensor 2 (Left): Humidity')
patch5 = mlines.Line2D([], [], color='red', marker = '^', label='Sensor 3 (Up): Temp')
patch6 = mlines.Line2D([], [], color='green', marker = '^', label='Sensor 3 (Up): Humidity')
...
def read_file(): #function that reads the csv and appends values to the deque arrays
    with open('data_' + save_time + '.csv', newline='') as f_input: #opens the file , comma delimiter default setting
        ...
        if len(t):
                list(takewhile(lambda row: datetime.strptime(row[0], '%d/%m/%Y %H:%M:%S') != t[-1], csv_input)) #recognises time stamp
...
def Plot(): #plotting function
 ...
 last_avgtemp = list(temp_avg)[-1]
 last_avghumid = list(humid_avg)[-1]
 ...
 #ax1.errorbar(t, y, yerr=1, fmt="none", color='b')
 ...
 #ax2.errorbar(t, h, yerr=1, fmt="none", color='b')
 ...
 plt.title('Average Temperature: ' +  "%.2f" % float(last_avgtemp) + '   ' + 'Average Humidity: ' + "%.2f" % float(last_avghumid))
...
def animate(i): #calls the 2 functions, is looped over by funcanimation every minute / x seconds
 read_file()
 Plot()
